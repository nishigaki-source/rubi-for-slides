...
1. `design/store-screenshot-1.png`(1280×800、RGB) — 設定パネルとルビが表示されたスライド編集画面の合成画像。「ローカライズ版」「全言語向け」の両方の欄にこのまま使える
2. ユーザー辞書の詳細設定ページ(表層形・読み・見た目を登録した状態) — 拡張機能アイコンのパネルから「ユーザー辞書を編集」を開いて撮影(任意)
