...
1. `design/store-screenshot-real-1.png`(1280×800、RGB) — 実際の画面から撮影・トリミングしたスクリーンショット。「ローカライズ版」「全言語向け」の両方の欄にこのまま使える(推奨)
2. `design/store-screenshot-1.png` — 上記が使えない場合の予備(実機UIを再現した合成画像)
3. ユーザー辞書の詳細設定ページ(表層形・読み・見た目を登録した状態) — 拡張機能アイコンのパネルから「ユーザー辞書を編集」を開いて撮影(任意)
4. (任意)「スライドへの書き込み」後、実際のテキストボックスとして書き込まれた状態

## 提出前のチェックリスト

- [x] Chrome ウェブストア デベロッパーアカウント登録($5)
- [ ] `npm run build:store-zip` で作った `rubi-for-slides.zip` をアップロード
      (`dist/` の `manifest.json` には `key` フィールドが残っているためストアには
      アップロードできない。このコマンドは `key` を除いた `dist-store/` から zip を作る。
      README.md の「Chromeウェブストアへの提出時は key フィールドが使えない」参照)
- [ ] アップロード後にストアが発行する拡張機能IDを確認し、Google Cloud Console の
      OAuthクライアントの「アプリケーションID」をそのIDに更新する(README.md 参照)
